...
import { ArrowLeft, Bot, Check, Copy, LoaderCircle, Send, Trash2, User } from 'lucide-react';
  ...
  const sendMessage = async (event?: FormEvent) => {
    ...
    try {
      ...
      const response = await apiFetch('/chat', {
        method: 'POST',
        body: JSON.stringify({ model: selectedModel, messages: history, max_tokens: 2048 }),
      });
      ...
      <div className="max-w-7xl w-full mx-auto flex-1 grid grid-cols-1 lg:grid-cols-[280px_minmax(0,1fr)] min-h-0">
        <aside className="border-b lg:border-b-0 lg:border-r border-slate-800 p-4">
          ...
          <button
            ...
            className="mt-3 w-full flex items-center justify-center gap-2 rounded-xl border border-slate-800 py-2.5 text-xs text-slate-400 hover:text-white hover:border-slate-600 disabled:opacity-40"
          >
            <Trash2 size={14} /> New conversation
          </button>
        </aside>
            ...
            <form onSubmit={(event) => void sendMessage(event)} className="max-w-3xl mx-auto">
              ...
              <div className="relative rounded-2xl border border-slate-700 bg-slate-900 focus-within:border-indigo-500 transition-colors">
                <textarea
                  ...
                  placeholder={modelReady ? `Message ${MODELS[selectedModel].short}…` : 'Waiting for model…'}
                  disabled={!modelReady || sending || switching}
                  rows={3}
                ...
                />
                <button type="submit" disabled={!input.trim() || !modelReady || sending || switching} aria-label="Send message" className="absolute right-3 bottom-3 w-9 h-9 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-800 disabled:text-slate-600 flex items-center justify-center transition-colors">
                  {sending ? <LoaderCircle size={17} className="animate-spin" /> : <Send size={17} />}
                </button>
              </div>
              <p className="mt-2 text-center text-[11px] text-slate-600">Enter to send · Shift+Enter for a new line · Conversation stays in this browser tab</p>
            </form>
          </div>
        </section>
      </div>
